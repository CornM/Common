...
public class EasyPhotosEngine implements ImageEngine {
    //单例
    private static EasyPhotosEngine instance = null;
    //单例模式，私有构造方法
    private EasyPhotosEngine() {
    }
    //获取单例
    public static EasyPhotosEngine getInstance() {
        if (null == instance) {
            synchronized (EasyPhotosEngine.class) {
                if (null == instance) {
                    instance = new EasyPhotosEngine();
                }
            }
        }
        return instance;
